...
function cook(ingr, opts) {
  ...
  // first ingredient is an option or flag > assume build command
  if (pan[0].substr(0, 1) === '-') {
    pan.unshift('build');

    config.verbose && logger.debug('Assumed build command: ' + 'ti '.yellow + utils.join(pan).yellow);
    ...
    if (config.commandAliases[pan[0]]) {
      ...
      pan[0] = config.commandAliases[alias];

      config.verbose && logger.debug('Resolved ' + alias.cyan + ' command alias: '.white + 'ti '.yellow + utils.join(pan).yellow);
    }
      ...
      if (matches) {
        ...
        if (name && recipes.has(name)) {
          ...
          if (doNotOverheat && _.contains(doNotOverheat, name)) {
            ...
            // only verbose when we're top-level
            config.verbose && doNotOverheat && logger.debug(prefix + '.. into: '.white + 'ti '.yellow + utils.join(before).yellow + ' ' + utils.join(recipe).cyan + ' ' + utils.join(after).yellow);
