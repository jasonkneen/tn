...
function cook(args, cfg, callback) {
  ...
  // catch and remove platform (prevents ambiguous options)
  if ((i = args.indexOf('-p')) !== -1 || (i = args.indexOf('--platform')) !== -1) {
    state.processed.platform = args[i + 1];
    args.splice(i, 2);
  }

  // prepend default recipe
  args.unshift.apply(args, recipes.get('_default'));
...
function lookup(arg, state) {
  ...
  if (platform_results_ln > 1) {
    ...
    return {
      platforms: _.properties(platform_results)
    };
  }
...
function rebuildArgs(state) {
  ...
  if (state.processed.platform === 'ios') {
    ...
    if (state.processed['developer-name'] && (state.processed.target === 'dist-playstore' || state.processed.target === 'dist-adhoc')) {
      state.processed['distribution-name'] = state.processed['developer-name'];
      delete state.processed['developer-name'];

    } else if (state.processed['distribution-name'] && state.processed.target !== 'dist-playstore' && state.processed.target !== 'dist-appstore') {
      state.processed['developer-name'] = state.processed['distribution-name'];
      delete state.processed['distribution-name'];
    }

    // setting default output dir for adhoc
    if (state.processed['target'] === 'dist-adhoc' && !state.processed['output-dir']) {
      state.processed['output-dir'] = (state.processed['project-dir'] || process.cwd()) + '/dist';
    }
  }
