...
function list(forReadMe) {
	...
	console.log('Recipes defined by: ' + ('built-in'.blue) + ', ' + ('user'.green) + ' and ' + ('user-override'.red));
	console.log();
...
function save(recipe, args) {

	if (!validateRecipeName(recipe)) {
		return;
	}

	var clr = _.has(recipes_system, recipe) ? 'red' : 'green';
...
function remove(recipe) {
	...
	if (_.has(recipes_user, recipe) === false) {
		logger.error('Unknown user recipe: ' + (recipe || '(none)').yellow);
		return;
	}
	...
	logger.info('Removed user recipe: ' + getRecipeName(recipe).yellow);
...
function rename(oldRecipe, newRecipe) {
	...
	if (_.has(recipes_user, oldRecipe) === false) {
		logger.error('Unknown user recipe: ' + oldRecipe.yellow);
		return;
	}
