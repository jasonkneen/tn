...
function list(forReadMe) {
	...
	console.log('Recipes defined by: ' + ('built-in'.blue) + ', ' + ('user'.green) + ', ' + ('project'.cyan) + ', ' + ('user-override'.red) + ' and ' + ('project-override'.yellow));
	console.log();
	...
	_.each(recipes, function(recipe) {
        ...
        if (_.has(recipes_project, recipe)) {
            color = _.has(recipes_system, recipe) || _.has(recipes_user, recipe) ? 'yellow' : 'cyan';
...
function save(recipe, args, location) {
	...
	var clr = _.has(recipes_system, recipe) ? 'red' : (location == 'project') ? 'yellow' : 'green';
    ...
    if (location == 'project' && _.has(recipes_project, recipe)) {
   	...
   	} else if (location == 'project' && _.has(recipes_system, recipe) && _.has(recipes_user, recipe)) {
        logger.info('Saved project recipe, overriding built-in and user');
    } else if (_.has(recipes_system, recipe)) {
   		logger.info('Saved ' + location + ' recipe, overriding built-in');
   	} else {
...
function remove(recipe, location) {
	...
	if (_.has(recipes_user, recipe) === false && _.has(recipes_project, recipe) === false) {
		logger.error('Unknown user or project recipe: ' + (recipe || '(none)').yellow);
		return;
	}
    ...
    logger.info('Removed ' + location + ' recipe: ' + getRecipeName(recipe).yellow);
